...
let texts = [
  {text: "Hello World", posX: 0, posY: 0, posZ: 0, textColor: 0xff0000},
  {text: "Another one", posX: 0, posY: 30, posZ: 0, textColor: 0x00ff00},
];
...
function getGlobalVectorFromOffset ({posX, posY, posZ, rotX, rotY, rotZ}) {
  ...
  globalVector.applyEuler(new THREE.Euler(rotX, rotY, rotZ, 'XYZ'));
  return globalVector;
}
function onDocumentMouseMove( event ) {
  ...
  if (mouseDown) {
    ...
    camera.rotation.x -= ( mouseY - event.clientY ) / window.innerHeight * 1 * Math.cos(camera.rotation.y);
    camera.rotation.z += ( mouseY - event.clientY ) / window.innerHeight * 1 * Math.sin(camera.rotation.y);
  }
...
document.addEventListener('keydown', (event) => {
  ...
  if (event.key === 'Backspace') {
    ...
    const offsetVector = getGlobalVectorFromOffset({posX: 0, posY: 0, posZ: -10, rotX: camera.rotation.x, rotY: camera.rotation.y, rotZ: camera.rotation.z});
    socket.emit('chat message', JSON.stringify({
      text: typedString,
      posX: camera.position.x + offsetVector.x,
      posY: camera.position.y + offsetVector.x,
  ...
  console.log(typedString); // Or display in an HTML element
...
function calcTexts() {
  ...
  toRender.forEach(({text, posX, posY, posZ, rotX, rotY, rotZ, textColor}) => {
    ...
    textMesh1.position.x = posX + centerOffset;
...
function showTypedText() {
  ...
  const offsetVector = getGlobalVectorFromOffset({posX: centerOffset, posY: 0, posZ: 5, rotX: camera.rotation.x, rotY: camera.rotation.y, rotZ: camera.rotation.z});
